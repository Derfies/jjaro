...
from jjaro.constants import NUM_SHAPE_COLLECTIONS
from jjaro.structures import BitmapHeader, Collection, CollectionHeader, Color
...
class ShpA:
    ...
    def load(self, file_path: str):
        """
        TODO: We probably don't want to do all the palette look ups etc here.
        This should be the thinnest wrapper around the data as possible, which
        allows downstream users to manipulate.
        ...
        with open(file_path, 'rb') as f:
            ...
            headers = []
            for _ in range(NUM_SHAPE_COLLECTIONS):
            ...
            for header in headers:
                ...
                for i in range(collection.bitmap_count):
                    ...
                    if bitmap_header.bytes_per_row > -1:
                        ...
                        bitmap_indices = np.frombuffer(raw, dtype=np.uint8).reshape(
                            (bitmap_header.height, bitmap_header.width))
                    else:

                        # This should be correct for width, height since numpy is row-major
                        bitmap_indices = np.zeros(
                            (bitmap_header.height, bitmap_header.width),
                            dtype=np.uint8)
                    ...
                    self.textures.append(colors[bitmap_indices])
